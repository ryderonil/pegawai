...
;(function($) {
	...
	Calendar.SHORT_DAY_NAMES = new Array('Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab');
	...
	Calendar.prototype = {
		...
		update: function(date) {
			...
			$('td.cal_title', this.container).html(Calendar.MONTH_NAMES[month] + ' ' + calendar.date.getFullYear());
		},
		...
		create: function(parent) {
			...
			// Calendar Table
			var table = $('<table />').addClass('cal_top');
			...
			*/
			...
			// Title Placeholder
			var row  = $('<tr />');
			this._drawButtonCell(row, '<span class="arrowside arrowleft">&nbsp;</span>', 1, Calendar.NAV_PREVIOUS_MONTH);
			var cell = $('<td colspan="5" class="cal_title" />');
			row.append(cell);
			this._drawButtonCell(row, '<span class="arrowside arrowright">&nbsp;</span>', 1, Calendar.NAV_NEXT_MONTH);
			thead.append(row);
			
			var table2 = $('<table />').addClass('cal_bottom');;
			var thead2 = $('<thead />');
			table2.append(thead2);

			// Day Names
			row = $('<tr />').addClass('day_title');
			for (var i = 0; i < 7; ++i) {
			...
			this.container = $('<div />').addClass('calendar').append(table).append(table2);
			if (this.isPopup) {
				this.container.css({
					position: 'absolute',
					display: 'none'
		...
		_drawButtonCell: function(parent, text, colSpan, navAction) {
			...
			cell.addClass('cal_button').html(text).attr('unselectable', 'on'); // IE;
			cell[0].calendar     = this;
		...
		showAtElement: function(element) {
			...
			this.showAt(offset.left + 40, offset.top + 122);
		},
