...
<table width=650>
  <tr>
     <!--  <td><input type=button value='Semua Data' onclick="window.location.href='<?php echo base_url(); ?>admin/list_jamaah';"></td> -->
    <td align="right"><form method="post" action="<?php echo base_url();?>umum/berdasarkan_paket">
 <input name="cari" type="text" value="<?php echo $this->session->userdata('peg');?>" /><input class="tombol" type="submit" name="submit" value="Cari" />
	...
	<?php 
		...
		$paket = $row['id_paket'];
		$a = $this->db->query("SELECT COUNT(`id_paket`) AS jumlah FROM jamaah WHERE `id_paket`='$paket' and id_kl='$klnya'");
	...
	<tr align="left">
		...
		<td align=center><?php 
		if($b !=0)
		echo anchor('umum/jumlah_jamaah_paket_user/'.$row['id_paket'], $row['nama_paket']);
		else
		echo $row['nama_paket'];
		?></td>
		...
		<td align=center><?php 
		if($b !=0)
		echo anchor('umum/jumlah_jamaah_paket_user/'.$row['id_paket'], $b);
		else
		echo $b;
		?>
		</td>
